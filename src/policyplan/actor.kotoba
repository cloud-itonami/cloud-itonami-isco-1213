(ns policyplan.actor
  "PolicyPlanningManagementActor — the ISCO-08 1213 community policyplan actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one policyplan operation request
  ...
  The unconditional invariant: the PolicyPlanningManagementAdvisor can never
  directly commit a record the PolicyPlanningManagementGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [policyplan.advisor :as advisor]
            [policyplan.governor :as governor]
            [policyplan.store :as store]))

(defn build-graph
  "Build a compiled PolicyPlanningManagementActor graph. `store` implements
  `policyplan.store/Store`. `advisor` implements
  `policyplan.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :control-point (:control-point proposal)
                                    :payload proposal}]
